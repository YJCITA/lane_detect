...
void LaneDetect::Init(Mat startFrame)
{
...
//         currFrame = Mat(320, 480, CV_8UC1, 0.0);     //initialised the image size to 320x480
	currFrame = Mat(640, 960, CV_8UC1, 0.0);     //initialised the image size to 320x480
			...
			if(contour_area > minSize){
				...
				if(bounding_length>longLane || bounding_width >longLane){
					...
					drawContours(temp2, contours,i, Scalar(255), CV_FILLED, 8);
							...
							(rotated_rect.center.y > vertical_top && rotated_rect.center.x > vertical_left && rotated_rect.center.x < vertical_right)))
				{
...
void LaneDetect::detectLane(Mat &nxt){
